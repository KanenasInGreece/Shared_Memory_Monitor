...
- **REM reliability · stranded & fairness** (gateway ≥0.7.x, decision 819; fairness ≥0.8.6 / fact 895 · decision 894) — from `telemetry.neo4j`: **Dead-lettered** (records retired at the retry cap) and **Failing** (still accumulating chargeable failures but not yet retired — the "stranded record" case: picked up repeatedly, never succeeded, never blamed, previously invisible), plus the **Retry cap**. Gateway ≥0.8.6 also surfaces **Passed over (yield)** and **Starved pending** (batch-vs-solo fairness / starved sub-queue). A nonzero stranded count is the safety net working as designed; fairness zeros under a thin backlog are honest dormancy (path not exercised).
