...
| **This release** | **v0.8.0** — **API v3** client · framework **≥ v0.7.0** (wire) · full Status + **diagram** LLM pool on **≥ v0.8.9** (**local/external** placement) |
...
**Compatibility (v0.8.0):** wire contract **API v3** (`X-SM-Api-Version: 3`) against **Shared Memory Framework gateway ≥ v0.7.0** — `./scripts/check-env.sh` / `agent-status.sh` should report `compat=ok`. For the full ops picture (Status + **diagram** LLM pool with **local** vs **external** badges, consolidation graph health + REM fairness instruments, latency drawer), run **gateway ≥ v0.8.9**; older gateways stay compatible and simply omit missing fields. Doctor prints which telemetry panels and placement signals are present.
...
| Client `X-SM-Api-Version` | Monitor **v0.8.0** advertises **api_version 3** — compatible with **framework ≥ v0.7.0**; full panel set + LLM placement on **≥ v0.8.9**. `./scripts/check-env.sh` reports `server=N client=N compat=ok` and which telemetry panels / placement signals are present. |
...
| [CHANGELOG.md](CHANGELOG.md) | Releases (current: **v0.8.0** · API **3** · framework **≥0.7.0** wire · **≥0.8.9** full panels) |
...
# gh release create v0.8.0 --title "v0.8.0" --notes-file …
```
