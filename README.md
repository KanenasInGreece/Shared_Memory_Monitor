...
| **This release** | **v0.9.9** — **API v4** client · framework **≥ v0.8.33** (wire) · alternative vectors on **≥ v0.8.40** · full Status + **diagram** LLM pool on **≥ v0.8.9** (**local/external** placement) |
...
**Compatibility (v0.9.9):** wire contract **API v4** (`X-SM-Api-Version: 4`) against **Shared Memory Framework gateway ≥ v0.8.33** — `./scripts/check-env.sh` / `agent-status.sh` should report `compat=ok`. **Alternative vectors** band (first-write quality) needs **gateway ≥ v0.8.40**. For the full ops picture (Status + **diagram** LLM pool with **local** vs **external** badges, consolidation graph health + REM fairness instruments, latency drawer), run **gateway ≥ v0.8.9**; older gateways stay compatible and simply omit missing fields. Doctor prints which telemetry panels and placement signals are present.
...
| [CHANGELOG.md](CHANGELOG.md) | Releases (current: **v0.9.9** · API **4** · framework **≥0.8.33** wire · **≥0.8.40** alt-vectors · **≥0.8.9** full panels) |
...
- [Shared Memory Framework](https://github.com/KanenasInGreece/Shared_Memory) — gateway, daemons, telemetry API (wire **≥0.8.33** / API 4; full monitor UI on **≥0.8.9**)
