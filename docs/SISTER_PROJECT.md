...
| `GET /pool/status` | Dream-job `free_slots` + per-backend `serves_all` / `counts_free_slot` (framework **≥0.9.13**; monitor v0.9.14+) |
...
**Client API version:** Monitor **v0.9.14** sets `bridge.API_VERSION = 4` to match
